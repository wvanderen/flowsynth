...
- **Module faces are readout panels** (issue #16, variant C; amended 2026-09-18, PR #67): the module type's **signature glyph is the centered centerpiece** at 0.8 scale; the contribution readout and its note sit beneath it, horizontally centered; the engraved level and the compact type nameplate sit above; and a narrow category-colored rail runs the face's edge. The chargeable face-hierarchy exception: a chargeable module's prominent readout is **charge-vs-threshold**, rendered as a threshold fill on its face with a flash/pulse at crossing.
...
- **The face hierarchy is amended (PR #67, 2026-09-18)**: the signature glyph is promoted from the original readout-first variant C to the face's centered centerpiece, with the contribution readout and note demoted beneath it and the level and nameplate shifted up. The glyph — not the readout — is now what a player scans for at board scale; the readout remains the face's numbers-first second glance. The chargeable threshold-fill exception is unchanged.
- **The Carrier's pin reads as panel-mount hardware (issue #94, PR #97, 2026-09-21)**: a bare white padlock marking — shackle arc over a rounded body, no housing plate — seated top-center above the engraved level and painted after the chassis so it stays legible, flanked by three engraved bolt circles at alternating chassis corners that skip the lock's top corner. The bolts are hardware, never a hue; the lock wears the Carrier's white neutral register. This is the panel language's one sanctioned literal pictogram: the lock is a status marking naming an invariant — pinned, immovable (ADR-0014) — not a category glyph, so the no-pictogram glyph law and the hue–glyph pairing are untouched. It supersedes the ticket's intermediate housed-badge sketch at the top-right.
