...
- Sleep gaps are sized by dual-clock drift: a positive step in `Date.now() − (performance.timeOrigin + performance.now())` past the quantization noise floor is a slept gap = away, even if the tab was "visible" when the machine slept; negative drift past the floor (clock rolled back) credits 0. Millisecond jitter between the two clocks' readings is measurement noise, not drift — a sub-floor step's boundary credits its wall gap whole and sizes no sleep. The floor sits an order of magnitude above that jitter and far below any real sleep or rollback, which move the drift by seconds.
...
- **Sleep**: no sleep event exists; sample dual-clock drift at every boundary — a positive step past the noise floor sizes the slept gap (§1). Chrome freeze (timers suspended, clocks running) appears the same way. `visibilitychange` covers tab switch, minimize, full obscuring, and screen off; focus loss alone does not hide.
