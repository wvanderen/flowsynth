...
*Scripted by [Onboarding beat sheet, #24](https://github.com/wvanderen/flowsynth/issues/24); amends #11's skeleton. The coarse skeleton, renumbered by the focus-tool elevation (→ [focus-tool spec](focus-tool-spec.md) §7, ADR-0019): grant → first upgrade; session one (open-ended, steered short); loud summary (nothing fires); generator pull; generator → charge → Forge → first roll; first cell purchase; capstone console long goal.*
...
3. **The surfaces around minute 0.** The catalog lists the starter shelf (Forge, generator, Additive Synth, infusor), each priced below reach with practice-minute countdowns; items vanish as acquired. There is no activation section: the ladder rests empty until Tasks (ADR-0019). The console rail shows all four tiles live — greyed tiles and locknotes are struck (ADR-0019) — each wearing its live state; tiles open nothing unusual to the script and the board never moves beneath them.
...
5. **Enter prompt.** "What are you practicing?" — the habit ask; it opens whenever no habit is selected, so session one always meets it, and once a habit is selected the Enter switch starts directly (→ ADR-0018). Unstructured practice stays a visible affordance of the prompt (no habit development accrues, nous unaffected). The duration affordances — preset chips plus free 1–90 entry — ride the prompt from the very first start, visible but unpushed: open-ended rests as the plan, and the reinstated session-one copy steers a short try (~5 minutes, tuning), then exiting. Session one can still be planned — nothing gates the chips (ADR-0019).
6. **Session one — flow.** HUD: ticking nous counter and per-practice-minute rate. No countdowns in-session, zero purchases possible. The focus tiles open in-session wearing live state but are never spotlighted — the script never points at them (ADR-0019). The beats are identical whatever the session's length or early exit.
7. **End — the loud summary.** A modal on returning to upgrade mode, however the session ended. Rows: headline "this session earned X nous" · practice minutes · rate achieved with the carrier-only breakdown · the unlock row in design but inert — nothing fires at launch, since the apps were already free (ADR-0019). No countdown rows. The reflection rides above dismissal (→ [focus-tool spec](focus-tool-spec.md) §8).
8. **Post-session — the generator pull.** The side-by-side fork is struck (ADR-0019): there is no rung-one alternative, and the catalog sells no apps. The shelf generator — its row carrying the one permanent hint, "produces charge — feeds the Forge" — is the only spend path; early agency lives in the free apps.
