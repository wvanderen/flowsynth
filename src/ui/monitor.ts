  ...
  if (host.dataset.renderKey !== key) {
    ...
    host.innerHTML = `
      <div class="monitor-top">
        ...
        ${ledgerOnMonitor ? featsChipHtml(feats) : ""}
      </div>
      ${accumulatorHtml(state, past)}`;
    byId("prestige-button")?.addEventListener("click", () => app.acknowledgeHorizon());
    if (ledgerOnMonitor) byId("feats-chip")?.addEventListener("click", () => app.openModal("achievements"));
  }
  if (ledgerOnMonitor) {
    updateLedgerLive(host, state, snapshot.rate);
  }
