...
import { renderStatusMonitor, formulaEquationHtml, formulaPanelHtml, updateFormulaLive } from "./monitor";
  ...
  if (strip) {
    ...
    if (strip.dataset.protoKey !== structKey) {
      ...
      if (variant === "a") {
        ...
        strip.innerHTML = ledgerFormulaHtml(formulaEquationHtml(achieving, infused), formulaPanelHtml(achieving, infused));
...
function renderBoardLedger(app: App): void {
  ...
  if (host.dataset.protoKey !== key) {
    ...
    const ledger = variant === "e"
      ? ledgerFormulaHtml(formulaEquationHtml(achieving, infused), formulaPanelHtml(achieving, infused))
      : ledgerHtml();
