...
// tab was "visible"; a negative step past the quantization noise floor
// (clock rolled back) credits zero, while sub-floor steps are measurement
// jitter the boundary ignores. Paused time is neither present nor away: it
// produces and credits nothing.
export function applyGap(
  ...
  // but only past the noise floor. Sub-floor steps are the two clocks'
  // millisecond quantization jittering the measurement, not drift; treating
  // each one as a rollback discarded that boundary's whole wall gap, and a
  // stopwatch against the session clock read a steady fraction of real time.
  if (driftStepSeconds < -DRIFT_NOISE_SECONDS) return ZERO;
  const slept = Math.min(Math.max(driftStepSeconds, 0), seconds);
